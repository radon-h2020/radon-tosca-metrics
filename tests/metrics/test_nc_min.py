...
@parameterized_class([
   ...
   { 'yaml': yaml_2_2, 'expected': 0},
   { 'yaml': yaml_5_1, 'expected': 2}
])

class TestNCMin(unittest.TestCase):
    def setUp(self):
    ...
    def test(self):
        metric = NC(self.yaml)
        minNb = metric.min()
        self.assertEqual(minNb, self.expected, 'Test failed because expected ' + str(self.expected) + ' and got ' + str(minNb) +'!') 
