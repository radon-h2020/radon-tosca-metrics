...
@parameterized_class([
   ...
   { 'yaml': yaml_5_1, 'expected': 3}
])

class TestNCMax(unittest.TestCase):
    def setUp(self):
    ...
    def test(self):
        metric = NC(self.yaml)
        maxNb = metric.max()
        self.assertEqual(maxNb, self.expected, 'Test failed because expected ' + str(self.expected) + ' and got ' + str(maxNb) +'!') 
