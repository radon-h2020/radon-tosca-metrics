...
@parameterized_class([
   ...
   { 'input': {'step1': WorkflowStepDefinition(target='Welcome.Stop')}, 'expected': False},
...
class TestImperativeWorkflowDefinitionSteps(unittest.TestCase):

    def test(self):
        raised = False

        try:
            ImperativeWorkflowDefinition(steps=self.input)
        except TypeError:
            raised = True
        finally:
