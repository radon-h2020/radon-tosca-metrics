...
class NIF(BlueprintMetric):
        ...
        try:
            ...
            for option in options:
                ...
                for element in option:
                    element_interfs = getInterfaces(element)
                    interfaces_list.extend(element_interfs)     
    ...
    def _get_counts_per_element(self):
        try:
            interfaces_list = self._get_elements()

            count_list = []
            for interfaces in interfaces_list:
                count_list.append(len(interfaces.keys()))
    ...
    def count(self):
        '''Function which counts the number of unique defined interfaces in a script''' 
        try:
            interfaces_list = self._get_elements()

            names = []
            for interfaces in interfaces_list:
                names.extend(interfaces.keys())
    ...
    def min(self):
        try:
            interface_count = self._get_counts_per_element()
            return min(interface_count)
...
# path = r'c:\Users\s145559\OneDrive - TU Eindhoven\School\JADS\Jaar 2\Thesis\RADON PROJECT\GIT projects\ANALYSIS\dataminer\tmp\aiorchestra\aiorchestra\Example\invalid_node_template-2.yaml'
...
# metric._get_counts_per_element()
